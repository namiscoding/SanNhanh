...
import { Loader2, Calendar, Users, MapPin, Clock, DollarSign, Check, Eye, ChevronLeft, ChevronRight, Search, SlidersHorizontal, Phone, Mail, Building2 } from 'lucide-react';
import {
...
// --- Hàm định dạng (có thể di chuyển ra một file utils.js nếu có nhiều nơi sử dụng) ---
...
const OwnerBookings = () => {
  ...
  const [filters, setFilters] = useState({
    status: '',
    // courtComplexId: '', // <<< ĐÃ XÓA DÒNG NÀY
    date: '',
    search: '',
    sortBy: 'createdAt',
    sortOrder: 'desc',
  ...
  // const [courtComplexes, setCourtComplexes] = useState([]); // <<< ĐÃ XÓA DÒNG NÀY
  ...
  // Hàm fetch danh sách booking từ API (Không đổi logic)
  const fetchBookings = useCallback(async () => {
    ...
    try {
      ...
      setError('Không thể tải danh sách đơn đặt sân.');
    } finally {
      setLoading(false);
    }
  }, [currentPage, filters]);

  // HÀM fetchComplexes ĐÃ ĐƯỢC XÓA VÌ KHÔNG CẦN NỮA
  // const fetchComplexes = async () => { /* ... */ }; 

  // useEffect để fetch danh sách khu phức hợp (ĐÃ BỊ XÓA)
  useEffect(() => {
    // fetchComplexes(); // <<< DÒNG NÀY SẼ BỊ XÓA
  }, []); // <<< DÒNG NÀY SẼ BỊ XÓA

  // Effect để fetch bookings khi currentPage hoặc filters thay đổi
  useEffect(() => {
    fetchBookings();
  }, [fetchBookings]);


  // Hàm xử lý khi thay đổi giá trị bộ lọc (text input, select)
  ...
  // Hàm xử lý khi thay đổi sắp xếp (sort by, sort order)
  ...
  // --- Hàm xử lý Action (Duyệt, Từ chối, Hoàn thành, Hủy, Xem chi tiết) ---
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <h1 className="text-3xl font-bold mb-6">Quản lý đơn đặt sân</h1>
      ...
      <Card className="mb-6">
        <CardContent className="p-6">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 items-end"> {/* ĐÃ THAY ĐỔI lg:grid-cols-4 THÀNH lg:grid-cols-3 */}
            {/* Search */}
            <div>
      ...
      <Card>
        <CardHeader>
          <CardTitle>Danh sách đơn đặt sân ({totalItems} đơn)</CardTitle>
        </CardHeader>
        <CardContent>
          {loading && bookings.length === 0 ? (
                ...
                <div key={booking.id} className="flex flex-col md:flex-row items-start md:items-center justify-between p-4 border rounded-lg shadow-sm">
                  <div className="flex-grow grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-y-2 gap-x-4 mb-4 md:mb-0">
                    ...
                    <div>
                      ...
                      {booking.customerEmail && <p className="text-sm text-gray-600 flex items-center"><Mail className="w-3 h-3 mr-1"/> {booking.customerEmail}</p>}
                      {booking.customerPhone && <p className="text-sm text-gray-600 flex items-center"><Phone className="w-3 h-3 mr-1"/> {booking.customerPhone}</p>}
                    </div>
                    {/* Price & Status */}
                    <div>
                    ...
                    <div>
                      ...
                      {booking.bookingType && <p className="text-xs text-gray-500 mt-1">Loại: {booking.bookingType === 'WalkIn' ? 'Vãng lai' : 'Online'}</p>}
                    </div>
                  </div>
            ...
            <div className="text-center py-8">
              ...
              <p className="text-gray-500">Không tìm thấy đơn đặt sân nào phù hợp.</p>
            </div>
          )}
        </CardContent>
        {/* Pagination */}
